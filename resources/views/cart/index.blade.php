    ...
    <div class="container py-4">
                                        ...
                                        <tr>
                                            <td class="ps-4" data-label="Product">
                                                <div class="d-flex align-items-center gap-3">
                                                    ...
                                                    <div class="text-start">
                                                        ...
                                                        <span class="badge bg-light text-dark border rounded-pill fw-medium small">Size: {{ $item->size }}</span>
                                                        
                                                        <form method="post" action="{{ route('cart.destroy', $item) }}" class="mt-2">
                                                            @csrf @method('DELETE')
                                                            <button type="submit" class="btn btn-link text-danger p-0 small text-decoration-none fw-bold">
                                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="me-1"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                                                                Remove
                                                            </button>
                                                        </form>
                                                    </div>
                                            ...
                                            <td data-label="Quantity">
                                                <form method="post" action="{{ route('cart.update', $item) }}" class="d-flex gap-2 align-items-center">
                                                    @csrf @method('PATCH')
                                                    <input type="number" name="quantity" min="1" max="99" value="{{ $item->quantity }}" class="form-control form-control-sm rounded-pill text-center px-3" style="width: 70px;" onchange="this.form.submit()">
                                                </form>
                                            </td>
                                            <td class="text-end pe-4" data-label="Subtotal">
                                                <span class="fw-bold text-primary fs-5">₹{{ number_format($itemPrice * $item->quantity, 2) }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                <div class="col-lg-4">
                    <div class="card border-0 shadow-sm rounded-4 p-4 sticky-top" style="top: 100px;">
                        ...
                        <div class="d-flex justify-content-between mb-5">
                            ...
                            <span class="fs-4 fw-bold text-primary">₹{{ number_format($subtotal, 2) }}</span>
                        </div>
                        
                        <a href="{{ route('checkout.show') }}" class="btn btn-dark btn-lg w-100 rounded-pill py-3 fw-bold shadow-sm mb-3">
                            Proceed to Checkout
                        </a>
                        
                        <div class="text-center">
                            <img src="{{ asset('images/secure-payment.png') }}" alt="Secure Payment" class="opacity-50" style="height: 30px; filter: grayscale(1);">
        ...
        @else
            <div class="py-5 text-center bg-light rounded-4 border border-dashed mt-4">
                <div class="py-5">
                    <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="1" class="mb-3"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>
                    <h3 class="fw-bold">Your bag is empty</h3>
                    <p class="text-muted mb-4">Items added to your bag will appear here.</p>
                    <a href="{{ route('shop.index') }}" class="btn btn-primary rounded-pill px-5 py-3 fw-bold shadow">Explore Catalog</a>
                </div>
            </div>
        @endif
    </div>
