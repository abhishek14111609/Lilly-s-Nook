                    ...
                    <form method="POST" action="{{ route('contact.store') }}">
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Jane Doe" value="{{ old('name') }}" required>
                                @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-uppercase">Email Address</label>
                                <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" placeholder="jane@example.com" value="{{ old('email') }}" required>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="message" class="form-control @error('message') is-invalid @enderror" rows="5" placeholder="Share your thoughts with us..." required>{{ old('message') }}</textarea>
                                @error('message') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-12">
                                <button type="submit" class="btn btn-dark btn-lg rounded-pill px-5 py-3 fw-bold shadow-sm mt-2">Send Message</button>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 p-4 bg-soft-warning text-warning">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mb-0 text-dark small">{{ $contactAddress ?? 'Boutique #404, Fashion Square, Mumbai' }}</p>
                            </div>
                        </div>
                    </div>
