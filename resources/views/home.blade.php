...
@section('content')
    ...
    <section class="py-5 reveal-section">
        <div class="container">
            <div class="d-flex justify-content-between align-items-end mb-5 flex-wrap gap-3">
                <div>
                    <h2 class="display-5 fw-bold mb-0">Shop The Look</h2>
                    <p class="text-muted fs-5 mb-0">Explore our curated collections</p>
                </div>
                <a href="{{ route('shop.index') }}" class="btn btn-outline-dark rounded-pill px-4 fw-bold">View All</a>
            </div>

            <div class="position-relative category-carousel-container">
                <!-- Carousel wrapper -->
                <div class="category-carousel-wrapper overflow-hidden" style="perspective: 1000px;">
                    <div class="category-carousel" id="categoryCarousel"
                        style="display: flex; gap: 1rem; transform: translateX(0); transition: transform 0.5s ease;">
                ...
                @if ($categories->count() > 3)
                    <button class="carousel-nav-btn carousel-nav-prev" id="categoryPrev" onclick="scrollCarousel(-1)"
                        style="position: absolute; left: -50px; top: 50%; transform: translateY(-50%); background: #333; color: white; border: none; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 10;">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="2">
                            <path d="M15 19l-7-7 7-7"></path>
                        </svg>
                    </button>
                    <button class="carousel-nav-btn carousel-nav-next" id="categoryNext" onclick="scrollCarousel(1)"
                        style="position: absolute; right: -50px; top: 50%; transform: translateY(-50%); background: #333; color: white; border: none; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 10;">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="2">
    ...
    <section class="py-5 bg-light reveal-section">
        <div class="container text-center py-4">
            <h3 class="playfair fs-2 mb-4 mx-auto" style="max-width: 800px;">"Inspired by whispers of the past, Lilly's Nook
                curates enchanting outfits that spark wonder in the hearts of curious girls."</h3>
            <div class="d-flex justify-content-center flex-wrap gap-2">
                @foreach (['Baby girl', '2-3 Years', '3-4 Years', '4-5 Years', '5-6 Years', '6-7 Years'] as $age)
                    <span
                        class="badge bg-white text-dark shadow-sm rounded-pill px-3 py-2 border fw-medium">{{ $age }}</span>
                @endforeach
            </div>
        </div>
    </section>
...
@push('scripts')
    <script>
        ...
        let currentScroll = 0;
        const cardWidth = 320 + 16; // 320px + 1rem gap
        ...
        function updateCarouselButtons() {
            ...
            if (prevBtn) prevBtn.style.opacity = currentScroll <= 0 ? '0.3' : '1';
            if (nextBtn) nextBtn.style.opacity = currentScroll >= maxScroll ? '0.3' : '1';
        }

        // Initialize on load
        document.addEventListener('DOMContentLoaded', updateCarouselButtons);
