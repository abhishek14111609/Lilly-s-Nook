...
@section('content')
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
        <div class="card-body p-3">
            <form action="{{ route('admin.products.index') }}" method="GET" class="row g-2">
                <div class="col-lg-4 col-md-6">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" name="search" class="form-control bg-light border-0 py-2"
                            placeholder="Search SKU or name..." value="{{ request('search') }}">
                    </div>
                </div>
                <div class="col-lg-2 col-md-3">
                    <select name="category_id" class="form-select form-select-sm bg-light border-0 py-2"
                ...
                <div class="col-lg-2 col-md-3">
                    <select name="stock_status" class="form-select form-select-sm bg-light border-0 py-2"
                        onchange="this.form.submit()">
                ...
                <div class="col-lg-4 col-md-12 d-flex gap-2">
                    <button type="submit" class="btn btn-dark btn-sm rounded-pill px-4 grow flex-md-grow-0">Filter</button>
                    <a href="{{ route('admin.products.index') }}"
                        class="btn btn-light btn-sm rounded-pill px-4 border grow flex-md-grow-0">Clear</a>
                </div>
            </form>
        </div>
    </div>
                        ...
                        <tr>
                            <td class="ps-4" data-label="Product">
                                <div class="d-flex align-items-center gap-3">
                                    <div class="rounded-3 border overflow-hidden d-flex align-items-center justify-content-center bg-white"
                                        style="width: 52px; height: 52px; flex-shrink: 0;">
                                        @if ($product->image)
                                            <img src="{{ asset('images/' . $product->image) }}"
                                                class="w-100 h-100 object-fit-cover" alt="">
                                        @elseif ($product->video)
                                            <video src="{{ asset(ltrim($product->video, '/')) }}"
                                    ...
                                    <div class="text-start">
                                        <div class="fw-bold text-dark text-truncate" style="max-width: 250px;">
                                            {{ $product->name }}</div>
                                        <div class="text-muted text-xs text-uppercase font-monospace">
                            ...
                            <td data-label="Status">
                                <span
                                    class="badge rounded-pill bg-soft-{{ $product->status == 'active' ? 'success' : 'secondary' }} text-{{ $product->status == 'active' ? 'success' : 'secondary' }} text-uppercase fw-bold px-3 py-1"
                            ...
                            <td class="text-end pe-4" data-label="Actions">
                                <div class="d-flex gap-2 justify-content-end">
                                    <a href="{{ route('admin.products.edit', $product) }}"
                                        class="btn btn-white btn-sm rounded-circle border shadow-sm p-2" title="Edit">
                                    ...
                                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            class="btn btn-white btn-sm rounded-circle border shadow-sm p-2 text-danger"
                                            title="Delete">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                                                stroke="currentColor" stroke-width="2">
                            ...
                            <td colspan="5" class="text-center py-5">
                                <div class="py-4">
                                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none"
                                        stroke="#dee2e6" stroke-width="1.5" class="mb-3">
                                    ...
                                    <a href="{{ route('admin.products.create') }}"
                                        class="btn btn-primary rounded-pill px-4">Create Product</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div class="text-muted small">Showing {{ $products->firstItem() ?? 0 }} to {{ $products->lastItem() ?? 0 }} of
            {{ $products->total() }} items</div>
