        ...
        <div class="row g-3">
            ...
            <div class="col-md-3">
                ...
                <select name="category_id" class="form-select" onchange="this.form.submit()">
                    ...
                    @foreach($categories as $parent)
                        <option value="{{ $parent->id }}" {{ request('category_id') == $parent->id ? 'selected' : '' }}>{{ $parent->name }}</option>
            ...
            <div class="col-md-3">
                ...
                <select name="price_range" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="5000+" {{ request('price_range') == '5000+' ? 'selected' : '' }}>Above ₹5,000</option>
                </select>
            </div>
        </div>
        <div class="row mt-3">
                    ...
                    <tr>
                        <td class="ps-4">
                            <div class="d-flex align-items-center gap-3">
                                <div class="product-avatar bg-soft-primary rounded d-flex align-items-center justify-content-center" style="width: 50px; height: 50px;">
                                    @if($product->image)
                                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="rounded" style="width: 100%; height: 100%; object-fit: cover;">
                                    @else
                            ...
                            @if($product->category)
                                <div>
                                    ...
                                    @if($product->category->parent)
                                        <div class="text-muted small mt-1">{{ $product->category->parent->name }}</div>
                                    @endif
                                </div>
                            @else
                        ...
                        <td>
                            ...
                            @if($product->sale_price && $product->sale_price < $product->price)
                                <div class="text-muted small text-decoration-line-through">₹{{ number_format($product->sale_price, 2) }}</div>
                            @endif
                        </td>
                        <td>
                            @php
                        ...
                        <td>
                            <div class="action-buttons justify-content-center">
                                                                <a href="{{ route('admin.products.edit', $product) }}" class="btn btn-secondary btn-sm" data-bs-toggle="tooltip" title="Edit Product">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                    </svg>
                                </a>
                                <button class="btn btn-warning btn-sm" data-bs-toggle="tooltip" title="Toggle Status" onclick="alert('Status toggle feature not available in current system')">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                ...
                                <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this product? This action cannot be undone.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-sm" data-bs-toggle="tooltip" title="Delete Product">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
